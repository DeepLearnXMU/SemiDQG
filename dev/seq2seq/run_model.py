...
class ModelArguments:
    ...
    use_posterior: bool = field(
        default=False,
        metadata={"help": "Whether to use dialogue response information."},
    )
    # mask_unseen_words: bool = field(
    #     default=False,
    #     metadata={"help": "Whether to use mask keywords in dialogue response."},
    # )
    # random_mask_ratio: float = field(
    #     default=0.4,
    #     metadata={"help": "Mask ratio for keywords in response."},
    # )
    do_lower_case: bool = field(
        default=False,
        metadata={"help": "Whether to use lower case."},
    )
...
class DataTrainingArguments:
    ...
    predict_output_file: Optional[str] = field(
        ...
        metadata={"help": "File name of predictions."},
    )
...
def main():
    ...
    if model_args.do_lower_case:
        tokenizer.add_tokens(["▁[mask]", "[mask]"])
    else:
        tokenizer.add_tokens(["▁[MASK]", "[MASK]"])
    ...
    if model_args.use_posterior:
        print("Use posterior knowledge from dialogue response..")
    else:
        print("Normal dialogue query generation..")
    ...
    def preprocess_function(examples):
        ...
        if model_args.do_lower_case:
            inputs = [str.lower() for str in inputs]
            targets = [str.lower() for str in targets]
    ...
    print("data_args.lang:", data_args.lang)
    ...
    def compute_metrics(eval_preds):
        ...
        result = metric.compute(
            ...
            use_stemmer=True,
            tokenizer=(lambda x: list(x)) if data_args.lang == "zh" else None,
        )
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                output_prediction_file = os.path.join(
                    training_args.output_dir, data_args.predict_output_file
                )
